...
import numpy as np
import pandas as pd
from utils import sigma_raw_fn, sigma_shrink_fn_capped
...
def run_sector_optimisation(sector_name, sector, R, cov_type="raw", cache_dir="cache"):
    ...
    elif cov_type == "shrink_capped":
        Sigma, shrinkage_alpha = sigma_shrink_fn_capped(R_clean)
    else:
...
def run_period(period_tag):
    ...
    for sector_name in data['GICS Sector'].unique():
        ...
        res_shrink = run_sector_optimisation(sector_name, sector, R, cov_type="shrink_capped", cache_dir=cache_dir)
    ...
    out_path = f"results/optimal_portfolios/optimal_portfolios_all_te_{period_tag}_capped.pkl"
...
all_diag_df.to_excel("results/optimal_portfolios/diagnostics_summary_all_periods_capped.xlsx", index=False)
